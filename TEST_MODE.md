...
1. **Зажать кнопку KEY (TEST_BTN) на плате BlackPill**
2. **Подать питание через USB** или нажать кнопку RESET
3. **Удерживать KEY** до момента инициализации системы (~1 секунда)
4. **Отпустить KEY** после появления изображения на дисплее

### Аппаратная конфигурация

- **Кнопка:** KEY - встроенная кнопка на плате BlackPill (рядом с USB)
- **Пин:** PA0 (GPIOA, Pin 0)
- **Режим:** INPUT с внутренней подтяжкой PULL-UP
- **Логика:** Active LOW (кнопка замыкает PA0 на GND)
- **Расположение:** Физическая кнопка уже распаяна на плате, дополнительных подключений не требуется
