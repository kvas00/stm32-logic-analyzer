...
// Print startup banner with firmware version
...
int main(void)
{
  ...
  if (oled->init()) {
    ...
    oled->update();
    // Note: Banner timeout handled in testTask (non-blocking)
  }
  ...
  // Note: Startup banner will be printed from testTask
  // after USB CDC enumeration (button press or 5 sec timeout)
